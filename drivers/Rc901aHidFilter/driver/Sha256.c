...
typedef unsigned int RC901A_UINT32;
typedef unsigned long long RC901A_UINT64;

typedef char RC901A_UINT32_MUST_BE_32_BITS[(sizeof(RC901A_UINT32) == 4U) ? 1 : -1];
typedef char RC901A_UINT64_MUST_BE_64_BITS[(sizeof(RC901A_UINT64) == 8U) ? 1 : -1];
...
typedef struct RC901A_SHA256_CONTEXT {
    ...
    size_t blockLength;
    RC901A_UINT64 totalLength;
    RC901A_UINT32 state[8];
} RC901A_SHA256_CONTEXT;

static const RC901A_UINT32 g_roundConstants[64] = {
...
static RC901A_UINT32 RotateRight(RC901A_UINT32 value, unsigned int count)
{
...
{
    RC901A_UINT32 words[64];

    for (size_t index = 0; index < 16U; ++index) {
        const size_t offset = index * 4U;
        words[index] =
            ((RC901A_UINT32)context->block[offset] << 24U) |
            ((RC901A_UINT32)context->block[offset + 1U] << 16U) |
            ((RC901A_UINT32)context->block[offset + 2U] << 8U) |
            (RC901A_UINT32)context->block[offset + 3U];
    }

    for (size_t index = 16U; index < 64U; ++index) {
        const RC901A_UINT32 first =
            RotateRight(words[index - 15U], 7U) ^
            RotateRight(words[index - 15U], 18U) ^
            (words[index - 15U] >> 3U);
        const RC901A_UINT32 second =
    ...
    RC901A_UINT32 a = context->state[0];
    RC901A_UINT32 b = context->state[1];
    RC901A_UINT32 c = context->state[2];
    RC901A_UINT32 d = context->state[3];
    RC901A_UINT32 e = context->state[4];
    RC901A_UINT32 f = context->state[5];
    RC901A_UINT32 g = context->state[6];
    RC901A_UINT32 h = context->state[7];

    for (size_t index = 0; index < 64U; ++index) {
        const RC901A_UINT32 sum1 =
            RotateRight(e, 6U) ^ RotateRight(e, 11U) ^ RotateRight(e, 25U);
        const RC901A_UINT32 choose = (e & f) ^ ((~e) & g);
        const RC901A_UINT32 temporary1 = h + sum1 + choose + g_roundConstants[index] + words[index];
        const RC901A_UINT32 sum0 =
            RotateRight(a, 2U) ^ RotateRight(a, 13U) ^ RotateRight(a, 22U);
        const RC901A_UINT32 majority = (a & b) ^ (a & c) ^ (b & c);
        const RC901A_UINT32 temporary2 = sum0 + majority;
...
{
    context->totalLength += (RC901A_UINT64)dataLength;
    ...
    )
{
    ...
    const RC901A_UINT64 bitLength = context->totalLength * 8U;
